...
public class ZESImpl implements ZES {
    ...
    private boolean DEBUG_MODE = Boolean.valueOf(config.getString("DEBUG_MODE")); //true;    
    /**
     * Return Device Name.
     * @return Device Name.
     */
    ...
    public Object[] exec(String cmd, String[] args, byte[] argsBytes) throws DriverException, UnsupportedDriverOperation {
        ...
        } else if (cmd.equalsIgnoreCase("getSyncEnd")) {
            res.add(this.zes.getSyncEnd());
        } else if (cmd.equalsIgnoreCase("envioMonitor")) {
            this.zes.envioMonitor();
        } else {
...
//            zes.sckUDP.bind();                  
     ...
     *   4 El equipo se esta sincronizando con la dll. 
     *   */
    public Map<String, String> PGetMP3() {
        logger.debug("EXECUTING : " + Thread.currentThread().getStackTrace()[1].getMethodName());
        ...
        res.put("Reproduciendo", Reproduciendo + "");
        res.put("Pista", MP3Song + "");
        res.put("NumPistas", MP3NFiles + "");
     ...
     *   4 El equipo se esta sincronizando con la dll. 
     *   */
    public boolean PGetAmplificador() {
        logger.debug("EXECUTING : " + Thread.currentThread().getStackTrace()[1].getMethodName());

        //Volcamos el estado del amplificador
        boolean Estado = stateAmp;
        //Miramos que tipo de error nos ha generado la libreria.
        return Estado;
        //Siempre es true porque leemos directamente desde la variable.
    }
...
//        zes = new ProtocoloComunicacion();
     ...
     *  @return
     *   */
    private boolean EjecutaProcedimientoSetComandos(int Procedimiento, String valor) {
        logger.debug("START : " + Thread.currentThread().getStackTrace()[1].getMethodName());
        boolean resul = false;
        double dummy = 0;
...
//        public int INDICE_INSERCION = 0;
         ...
         *  
         *   */
        public static final int TIEMPODESCONEXION_POR_DEFECTO = 1000;
        /**
        ...
        private final int TIEMPO_ENVIO = 100;
        /**
...
//        private int Indice_Refresco;
        ...
        private String monitorEquipo;
        //  "Eventos de la clase"
        private DataEvent deo = new DataArrival();
        private boolean finSincronizacion = false;

        public boolean getSyncEnd() {
            return finSincronizacion;
        }
...
//                TmrEnvio = null;
        ...
        public void Conectar() {
            ...
            if (estadoComunicaciones != ZESStatus.SIN_CONEXION) { /*TODO antes == */
                ...
                if (!(comandoEquipoCorrecto.Comando == null) && !(comandoEquipoCorrecto.Respuesta == null)
                    ...
                    tmrEquipo.start();
//                    estadoComunicaciones = ZESStatus.CONECTANDO; /*TODO antes descomentado */
...
//                versionEquipo = "";
                ...
                if (tmrEquipoDesconectado != null) {
                    tmrEquipoDesconectado.stop();
                }
                iniciaColas();
                InicializaVariables();
        ...
        public void AgregaComandoPrio(String comand) {
            ...
            try {
                if (comand.startsWith("F")) {
                    ...
                    i = 0;
                    while (i <= numComandos) {
                        if (/*comand == colaEnvioPrio[i]*/colaEnvioPrio.get(i).equals(comand)) {
                            //colaEnvioPrio.Remove(i);
                            colaEnvioPrio.remove(i);
                            i--;
                            numComandos--;
        ...
        /**
         * Envío comando monitor.
         */
        public void envioMonitor() {
//            sckUDP.sendData(zes.START_TX + CobraNetUtil.COMANDO_MONITOR + zes.END_TX);
//            sckUDP.bind();
            EnviaPorUDP(CobraNetUtil.COMANDO_MONITOR);
            if ((monitorEquipo != null) && (!monitorEquipo.isEmpty())) {
                try {
                    ProcesComandoMonitor(monitorEquipo);
                } catch (CierreEntradaActivo ex) {
...
//            tmrEquipo.Enabled = false;
                    tmrEquipo.stop();
                    if ((comandoEquipoCorrecto != null) && (!(comandoEquipoCorrecto.Comando).isEmpty())) {
...
//                            TmrEnvio.start();
                ...
                finSincronizacion = true;
                TmrEnvio.start();
                Enviando = false; // TODO TEST
                SncRo = true;
                SncRw = true;
            }
        }
...
//            this.socket_DataArrival(Integer.parseInt(Short.toString(size)));                              
                ...
                if (cadena.startsWith("e") && cadena.endsWith("@")) {
                    versionEquipo = cadena;
                }
                if (cadena.startsWith("m") && cadena.endsWith("@")) {
                    monitorEquipo = cadena.substring(1, cadena.length() - 1);
                }
                /*TODO*/ Enviando = false;
...
//                tmrTimeOut.Enabled = false;
                    tmrTimeOut.stop();
                    nIntentos = 0;
                    /* TODO */ estadoComunicaciones = ZESStatus.OK;
                } else {
                    return;
                }
